...
class NMT(nn.Module):
    ...
    def __init__(self, word_embed_size, hidden_size, vocab, dropout_rate=0.3, no_char_decoder=False):
        ...
        self.decoder = nn.LSTMCell(input_size=(word_embed_size + hidden_size), hidden_size=self.hidden_size, bias=True)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        X_pack = pack_padded_sequence(X, source_lengths, enforce_sorted=False)
        lstm_out, (last_hidden, last_cell) = self.encoder(X_pack)
        lstm_out = pad_packed_sequence(lstm_out)
        enc_hiddens = lstm_out[0].permute((1, 0, 2))

        batch_size = last_hidden.shape[1]
        init_decoder_hidden = self.h_projection(last_hidden.permute((1, 0, 2)).reshape(batch_size, -1))
        init_decoder_cell = self.c_projection(last_cell.permute((1, 0, 2)).reshape(batch_size, -1))
