...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_weight = nn.Parameter(torch.rand(self.n_features * self.embed_size, self.hidden_size))
        self.embed_to_hidden_bias = nn.Parameter(torch.rand(self.hidden_size))
        nn.init.xavier_uniform_(self.embed_to_hidden_weight)
        nn.init.uniform_(self.embed_to_hidden_bias)

        self.dropout = nn.Dropout(self.dropout_prob)

        self.hidden_to_logits_weight = nn.Parameter(torch.rand(self.hidden_size, self.n_classes))
        self.hidden_to_logits_bias = nn.Parameter(torch.rand(self.n_classes))
    ...
    def forward(self, w):
        ...
        h = torch.nn.functional.relu(torch.matmul(x, self.embed_to_hidden_weight) + self.embed_to_hidden_bias)
        l = torch.matmul(h, self.hidden_to_logits_weight) + self.hidden_to_logits_bias
        logits = self.dropout(l)
